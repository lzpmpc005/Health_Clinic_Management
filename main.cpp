...
public:
    ...
    MedicalHistory(int id,  string date,  string history) : P_id(id), Date(date), P_history(history) {}
};
...
void add_prescriptions(MedicalHistory& m){
    sqlite3* db;
    sqlite3_open("medical_history.db", &db);

    string sql = "CREATE TABLE IF NOT EXISTS medical_history (id INTEGER PRIMARY KEY, date TEXT, history TEXT);";
    sqlite3_exec(db, sql.c_str(), NULL, NULL, NULL);

    sql = "INSERT INTO medical_history (id, date, history) VALUES (" + to_string(m.P_id) + ", '" + m.Date + "', '" + m.P_history + "');";
    sqlite3_exec(db, sql.c_str(), NULL, NULL, NULL);
...
json::wvalue print_medical_history(int patientId) {
    sqlite3* db;
    sqlite3_stmt* stmt;

    sqlite3_open("medical_history.db", &db);

    string sql = "SELECT * FROM medical_history WHERE id = " + to_string(patientId) + " ORDER BY date;";
    sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, NULL);

    json::wvalue medical_history;
    int index = 0;
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        int id;
        id = sqlite3_column_int(stmt, 0);
        string date = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1));
        string history = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));

        medical_history[index]["id"] = id;
        medical_history[index]["date"] = date;
        medical_history[index]["history"] = history;
        index++;
    }
...
int main() {
    ...
    CROW_ROUTE(hcm, "/add_prescriptions").methods("POST"_method)
        ([&](const request& req) {
                auto data = json::load(req.body);
                if (!data || !data.has("patient_id") || !data.has("date") || !data.has("history")) {
                    return response(400, "Invalid data");
                }
                ...
                MedicalHistory medicalHistory(id, date, history);
                medicalHistory.P_id = patientId;

                add_prescriptions(medicalHistory);

                json::wvalue response_data;
                response_data["id"] = id;
                response_data["message"] = "Prescription Successfully Added";

                return response(200, response_data);
            });

    CROW_ROUTE(hcm, "/medical_history/<int>").methods("GET"_method)
        ([&](const request& req, int patientId) {
            return response(200, print_medical_history(patientId).dump());
        });
